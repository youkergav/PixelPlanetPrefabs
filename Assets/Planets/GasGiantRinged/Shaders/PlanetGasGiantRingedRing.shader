Shader "Planet/GasGiantRinged/Ring"
{
    Properties
    {
	    ...
	    _LightNorder2("Light Border 2", range(0.0, 1.0)) = 0.62
    	_RingWidth("Ring Width", range(0.0, 0.15)) = 0.1
	    _RingPerspective("Ring Perspective", float) = 4.0
    	_PlanetScaleRatio("Planet Scale Ratio", float) = 6.0
	    
        _GradientTex2("Gradient Light", 2D) = "white" {}
    	_GradientTex3("Gradient Dark", 2D) = "black" {}
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag(v2f i) : COLOR {
				...
				float ring = smoothstep(0.5-_RingWidth*2.0, 0.5-_RingWidth, center_d);
				ring *= smoothstep(center_d-_RingWidth, center_d, 0.4);
				
				// pretend like the ring goes behind the planet by removing it if it's in the upper half.
				if (uv.y < 0.5) {
					ring *= step(1.0/_PlanetScaleRatio, distance(uv,float2(0.5,0.5)));
				}
				
				// rotate material in the ring
				uv_center = rotate(uv_center+float2(0, 0.5), _Timestamp*_Speed);
				// some noise
				...
				float3 col;
				if (posterized <= 1.0) {
					col = tex2D(_GradientTex2, float2(posterized, uv.y)).rgb;
				} else {
					col = tex2D(_GradientTex3, float2(posterized-1.0, uv.y)).rgb;
				}
