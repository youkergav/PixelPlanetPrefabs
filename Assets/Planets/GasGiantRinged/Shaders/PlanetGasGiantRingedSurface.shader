Shader "Planet/GasGiantRinged/Surface"
{
    Properties
    {
	    ...
	    _Light_border_1("Light Border 1", range(0.0, 1.0)) = 0.52
	    _Light_border_2("Light Border 2", range(0.0, 1.0)) = 0.62
    	_Bands("Bands", float) = 1.0

        _GradientTex2("Gradient Light", 2D) = "white" {}
    	_GradientTex3("Gradient Dark", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed4 frag(v2f i) : COLOR {
				...
				float fbm2 = fbm(uv*float2(1.0, 2.0)*_Size+fbm1+float2(-_Timestamp*_Speed,0.0)+turb);
